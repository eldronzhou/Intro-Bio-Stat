...
which(b %in% c("Biostatistics","Genetics")) # convert a logical vector to numerical index
...
We will start by introducing the idea of vectorization. We can use for loop to calculate the square root of a vector $a$. Alternatively, we can apply the function `sqrt` directly to the vector $a$. Many function in R can directly handle vector/matrix as the input by applying the operation implicitly to each element of the vector/matrix simultaneously. The second way is preferred as 1) the code is shorter, 2) it is faster although the speed difference is negligible in our example.
...
Another example would be adding two vector together. We can see that R implicitly adds each element of $x$ and $y$. Alternatively, we would be using a for loop to perform the test. Note that this also applied to subtraction *-*, multiplication ***, division */*, power *^*, and mathematical function such as taking exponential *exp* and logarithms *log*. 
...
Check point: Convert the following temperature to Celcius without using for loop.

```{r}
temp = c(35, 88, 42, 84, 81, 30)
```

### Some useful tricks and general advice

Use tab to auto-complete.

Use arrow in the console to navigate code.

If your code has for loop and function, format it with indentation.

Practice.

If you use AI to complete your homework, my advice is to understand it first. Then walk away to watch an episode of your favorite show (mine are Breaking Bad and The Wire). When you come back and open the problem again, can you rewrite it without the use of AI? 
