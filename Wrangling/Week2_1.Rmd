...
```{r}
dat = read_csv("/Users/gz/Desktop/Course/STAT59800/Week2/data/usa_covid.csv") # use tab for auto-completion
```
...
```{r}
write_delim(dat, file="data/usa_covid.txt", delim="\t", col_names=TRUE)
write_csv(dat, file="data/usa_covid.csv", col_names=TRUE)
```
...
Finally, let’s summarize the data by dividing it into two groups: dates when the total vaccination percentage was greater than 50%, and dates when it was less than 50%. For each group, we calculate the daily death rate and then compare the average daily death rates between the two groups.
...
dat %>% group_by(vacc_group) %>% 
    summarise(average=mean(death_rate_per_day, na.rm=TRUE), med=median(death_rate_per_day, na.rm=TRUE))
```
